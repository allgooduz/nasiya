    ...
    public function add(Request $request)
    {
        ...
        if (!empty(auth()->user())) {
            ...
            if (!empty($request->passport_main_string)) {
                Helper::storeImage($checkPhone, 'passport_main_string', 'passports');
            }

            if (!empty($request->passport_address_string)) {
                Helper::storeImage($checkPhone, 'passport_address_string', 'passports');
            }
        }else{
            ...
            if (empty($checkPhone)) {
                ...
                if (!empty($request->passport_main_string)) {
                    Helper::storeImage($newUser, 'passport_main_string', 'passports');
                }

                if (!empty($request->passport_address_string)) {
                    Helper::storeImage($newUser, 'passport_address_string', 'passports');
                }
            }else{
                if (!empty($request->passport_main_string)) {
                    Helper::storeImage($checkPhone, 'passport_main_string', 'passports');
        ...
        if (!empty(auth()->user()->id)) {
            $checkPhone = User::where("phone_number", auth()->user()->phone_number)->first();
        }else{
            $checkPhone = User::where("phone_number", $newPhone)->first();
        }
        ...
        if (!$checkPhone->passport_main_string) {
            ...
            $mainPassportToSend = env("APP_URL") . "/storage/". $checkPhone->passport_main_string;
        }
        ...
        if (!$checkPhone->passport_address_string) {
            ...
            $addressPassportToSend = env("APP_URL") . "/storage/". $checkPhone->passport_address_string;
        }
        ...
        return redirect()->to($order->url)->withSuccess(__('main.order_accepted'));
    }
